document.addEventListener("DOMContentLoaded", function () {
    ...
    // --- VERSION CONTROL ---
    console.log("Patro Layout Loaded - Version 11 (Auto-Cursor Inject)");

    // --- 0. GESTION DU CURSEUR (PRIORITAIRE) ---
    // On vérifie si c'est un PC (souris fine)
    if (window.matchMedia("(pointer: fine)").matches) {
        
        // On cherche le curseur
        let cursor = document.getElementById('custom-cursor');
        
        // S'il n'existe pas, ON LE CRÉE ! (Sécurité anti-bug)
        if (!cursor) {
            console.log("Curseur introuvable dans le HTML, création automatique...");
            cursor = document.createElement('div');
            cursor.id = 'custom-cursor';
            // Les styles sont gérés par style.css grâce à l'ID
        ...
        if(cursor) {
            document.addEventListener('mousemove', (e) => {
                ...
                requestAnimationFrame(() => {
                    ...
                    cursor.style.opacity = '1'; // Force la visibilité au mouvement
                });
            });

            // Cacher si on sort de la fenêtre
